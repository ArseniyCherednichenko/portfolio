...
- NEXT: real screenshots/visuals if/when available (posters are an honest stand-in, not a claim). A short no-backend contact note/form could still live on the `/contact` page. Consider surfacing Contact as a real Nav link if it earns the space. Could wire the `Lightbox` into the Playground gallery too.
