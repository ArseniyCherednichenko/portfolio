...
Aurora (ambient bg), GradientText (shine), RotatingWord, SpotlightCard (cursor glow), Marquee, MagneticButton, Nav (route-aware), Reveal, HeroOrbit, ScrollCue, Modal, Layout (shell + page transitions), TiltCard (3D cursor tilt + glare), AnimatedCounter (count-up on view), Eyebrow (shared lime section label), CommandPalette (global Cmd+K), ProjectPoster (deterministic generative brand art per project), Lightbox (focused fullscreen viewer). Homepage composed in `src/pages/Home.tsx`. Keyframes (`shine`, `marquee`) + reduced-motion guard live in `src/index.css`.
