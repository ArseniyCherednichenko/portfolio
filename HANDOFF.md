...
- Pages live in `src/pages/`: `Home.tsx` (hero/about teaser/work/toolkit/contact + project modal), `About.tsx` (intro, Now block, How-I-work principles, Path timeline, CTA), `Playground.tsx` (live motion experiments gallery), `WorkDetail.tsx` (`/work/:slug` project case study), `NotFound.tsx` (404).
- Routes: `/` Home, `/about` About, `/playground` Playground, `/work/:slug` WorkDetail, `*` NotFound. The Work anchor uses `/#work` so it works from any page; About and Playground are real page links in Nav.
...
- NEXT: a Contact page/dialog, an image lightbox, screenshots/visuals on the project detail pages, and a `/work` index page listing all case studies.
