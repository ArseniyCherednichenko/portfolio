...
- Pages live in `src/pages/` (Home, **Work** — the archive index, WorkDetail, About, **Now**, Playground, Uses, NotFound). Project content is centralized in `src/data/projects.ts` (used by the home work grid, the `/work` archive, case-study pages, and command palette). Add real projects there.
- `/work` is the editorial archive: a filterable table-of-contents of every project (filter chips derived from the union of project stacks, animated via a shared `layoutId`; rows reflow with Framer `layout`/`AnimatePresence`, reduced-motion aware). It de-centers any single project — Guided is one row among the body of work. Reuses `openProjectQuickLook` for real entries and renders `soon` placeholders as muted "in progress" rows. The home Work section is a teaser that links here; the command palette has a "Work archive" entry; WorkDetail breadcrumbs point back to `/work`. The nav "Work" link still scroll-spies the homepage `#work` section.
- `/now` is a present-tense snapshot (the [nownownow.com](https://nownownow.com/about) convention) that de-centers any single project by foregrounding the person: a short, honest "what has my attention" list. It is anchored by `LocalClock`, a live Berlin wall-clock (updates every second, blinking separator + pulsing dot, all reduced-motion aware) that reads back an honest, hedged status ("Probably asleep" / "Likely heads-down building") derived from the local hour via `Intl` with `timeZone: 'Europe/Berlin'`, so it is correct for any visitor. Linked from nav, footer, and the command palette. Keep the focus copy honest and rewrite it when life moves — that is the point of a now page.
...
Aurora (ambient bg), GradientText (shine), RotatingWord, SpotlightCard (cursor glow), Marquee, MagneticButton, Nav, Reveal, **DotGrid** (cursor-reactive canvas dot field, no particle lib — featured on /playground), **Scramble** (decode-on-view text, also drives Home's section eyebrows; `hoverable` replays on hover), **TextPressure** (cursor-reactive typography — each letter swells, bolds, and lights toward the lime accent by pointer proximity via one rAF loop, no per-letter React state; featured on /playground, renders static text under reduced motion). The full component set is larger — `ls src/components` before adding. Homepage composed in `src/pages/Home.tsx` (hero, about, work, toolkit, playground, approach, contact, faq). Keyframes (`shine`, `marquee`) + reduced-motion guard live in `src/index.css`.
