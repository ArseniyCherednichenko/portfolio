...
- Pages live in `src/pages/` (Home, **Work** — the archive index, WorkDetail, About, Playground, Uses, NotFound). Project content is centralized in `src/data/projects.ts` (used by the home work grid, the `/work` archive, case-study pages, and command palette). Add real projects there.
