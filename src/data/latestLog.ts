...
export const LATEST_LOG: LogHeadline[] = [
  { title: "A Times table — the single most surprising picture in elementary arithmetic, drawn as thread wound between pins. Space a few hundred points evenly round a circle and number them zero, one, two; then for a multiplier k run a straight thread from every point i across to point i times k, wrapping past the ring — that is, modulo the count. No curve is ever drawn, only chords, and yet the thread leaves a bright envelope: a cardioid at k of two, a nephroid at three, a chain of one-less-than-k cusps for every whole number after — the epicycloids a coin rolling round a coin would trace. Slide k off the whole numbers and the cusps unstitch and re-stitch. It reads as string art because it is: nails round a ring, one thread, a rule for where it goes next, tinted teal near zero and warming to lime round the ring, the chords laid in additively so their crossings glow. Left alone the multiplier climbs slowly through the whole numbers and back; drag across to drive k and up and down to add or thin the pins. Reduced motion settles one multiplier and paints a still figure", tag: 'Component' },
  { title: "A Superformula — one equation, drawn over and over, that turns two numbers into every shape between a circle and a starfish. Gielis's formula returns a radius for each angle; the parameter m sets the symmetry, so sweeping it from three to sixteen walks the outline unbroken through triangles, flowers, gears, stars and rosettes, while n1 swells the lobes into soft petals or draws them to needle spikes. Drawn as a bloom of nested rings, teal at the core warming to lime at the rim, counter-rotating so it reads as a living rose window. Left alone the symmetry breathes on its own; drag across to drive m and up and down to drive n1, and the whole bloom morphs under your hand. Reduced motion settles one pleasing symmetry and paints a still bloom", tag: 'Component' },
  { title: 'A Circle packing — the other way to fill a plane, and the counterpart to the Voronoi beside it: where that partitions space into cells that share every edge, this leaves the gaps and lets them fill themselves. A seed drops at a point clear of every circle already down, then grows outward until its rim kisses a neighbour or the wall and freezes — so big discs land where there was room and a lace of ever-smaller ones packs the slivers between, none overlapping, the size hierarchy tinted from cool teal to lime. The cursor carves: press or drag to lift the circles out and the packing grows back into the hole, smaller and denser, so froth heals behind you. Reduced motion computes one complete packing and paints it still', tag: 'Component' },
  { title: "A Bio page — 'The short version', the copyable, introduce-me artifact a public profile needs, and a plain statement of the person rather than any one project. A segmented control picks a length — one line, short, or full — and the bio cross-fades between three honest, third-person paragraphs, each with a live word and character count and a copy button that raises a toast. A fast-facts panel sits alongside where every line copies on its own tap. Only verifiable things, nothing invented; wired into the palette, the footer, the contents index and the 'go o' chord. Reduced motion drops the cross-fade and the presence ping", tag: 'Page' },
