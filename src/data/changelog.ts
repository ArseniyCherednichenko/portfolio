...
export const CHAPTERS: Chapter[] = [
  {
    ...
    items: [
      { kind: 'component', text: 'GooeyText — one word melts into the next: two text layers sit under a gooey SVG filter (a Gaussian blur, then an alpha-crushing matrix — the same metaball trick GooeyTabs plays on its blobs), so as one word blurs out and the next sharpens in, their glyph-halos fuse into a single liquid mass at the midpoint before resolving into clean type. Not a crossfade but a physical morph, and distinct from GooeyTabs (a moving pill) and MetaBalls (drifting blobs) — here the fusing shapes are letters. It replaced the plain crossfade on the hero’s "I build ..." line, the most-read line on the site, and is a Type & text experiment. Honest to assistive tech via an aria-live region; reduced motion drops the filter for an instant, legible swap' },
